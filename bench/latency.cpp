...
int main(int argc, char* argv[])
{
    ...
    benchmark::RegisterBenchmark("disabled-at-compile-time (global logger)",
                                 bench_disabled_macro_global_logger,
                                 disabled_logger);
    ...
    if (full_bench)
    {
        ...
        benchmark::RegisterBenchmark("basic_st/backtrace", bench_logger, std::move(tracing_basic_st))
            ->UseRealTime();
        ...
        auto tracing_rotating_st = spdlog::rotating_logger_st(
            ...
            file_size,
            rotating_files);
        benchmark::RegisterBenchmark("rotating_st/backtrace", bench_logger, std::move(tracing_rotating_st))
            ->UseRealTime();
        ...
        benchmark::RegisterBenchmark("daily_st/backtrace", bench_logger, std::move(tracing_daily_st))
            ->UseRealTime();
