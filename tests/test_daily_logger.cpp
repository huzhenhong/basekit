...
TEST_CASE("daily_logger with dateonly calculator", "[daily_logger]")
{
    ...
    spdlog::fmt_lib::format_to(std::back_inserter(w), SPDLOG_FILENAME_T("{}_{:04d}-{:02d}-{:02d}"), basename, tm.tm_year + 1900, tm.tm_mon + 1, tm.tm_mday);
...
struct custom_daily_file_name_calculator
{
    ...
    {
        filename_memory_buf_t w;
        spdlog::fmt_lib::format_to(std::back_inserter(w), SPDLOG_FILENAME_T("{}{:04d}{:02d}{:02d}"), basename, now_tm.tm_year + 1900, now_tm.tm_mon + 1, now_tm.tm_mday);
...
TEST_CASE("daily_logger with custom calculator", "[daily_logger]")
{
    ...
    std::tm               tm       = spdlog::details::os::localtime();
    filename_memory_buf_t w;
    spdlog::fmt_lib::format_to(std::back_inserter(w), SPDLOG_FILENAME_T("{}{:04d}{:02d}{:02d}"), basename, tm.tm_year + 1900, tm.tm_mon + 1, tm.tm_mday);
...
TEST_CASE("daily_file_sink::daily_filename_format_calculator", "[daily_file_sink]")
{
    ...
    auto    filename = spdlog::sinks::daily_filename_format_calculator::calc_filename(
        SPDLOG_FILENAME_T("example-%Y-%m-%d.log"),
        tm);
