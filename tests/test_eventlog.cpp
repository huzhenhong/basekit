                                      ...
                                      WORD                                    expected_ev_type)
{
    ...
    auto  ok = ::ReadEventLogA(event_log.handle_, EVENTLOG_SEQUENTIAL_READ | EVENTLOG_BACKWARDS_READ, 0, &read_bytes, 0, &read_bytes, &size_needed);
    REQUIRE(!ok);
    ...
    ok = ::ReadEventLogA(event_log.handle_, EVENTLOG_SEQUENTIAL_READ | EVENTLOG_BACKWARDS_READ, 0, record, size_needed, &read_bytes, &size_needed);
    REQUIRE(ok);
...
TEST_CASE("eventlog", "[eventlog]")
{
    ...
    test_single_print([&test_logger](std::string const& msg)
                      { test_logger.trace(msg); },
                      "my trace message",
                      EVENTLOG_SUCCESS);
    test_single_print([&test_logger](std::string const& msg)
